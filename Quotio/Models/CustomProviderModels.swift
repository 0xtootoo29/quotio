...
extension CustomProvider {
    ...
    var effectiveModelMappingsForExport: [ModelMapping] {
        ...
        var mappings = models
        let discoveredModelIDs = discoveredRelayModelIDsFromCatalog

        // If real models are discovered, suppress legacy Claude slot aliases
        // (relay-slug-opus/sonnet/haiku) to avoid masking real upstream names.
        if !discoveredModelIDs.isEmpty, let relaySlug = relayChannelSlug {
            let legacySlotAliases = Set([
                "\(relaySlug)-opus",
                "\(relaySlug)-sonnet",
                "\(relaySlug)-haiku"
            ])
            mappings.removeAll { mapping in
                legacySlotAliases.contains(mapping.effectiveAlias.lowercased())
            }
        }

        // Migrate legacy aliases to the current generated alias scheme.
        if let relaySlug = relayChannelSlug, !mappings.isEmpty {
            let legacyRelaySlug = legacyRelayChannelSlug
            mappings = mappings.map { mapping in
                var next = mapping
                let modelID = mapping.name.trimmingCharacters(in: .whitespacesAndNewlines)
                guard !modelID.isEmpty else { return next }

                let normalizedExistingAlias = mapping.alias
                    .trimmingCharacters(in: .whitespacesAndNewlines)
                    .lowercased()
                guard !normalizedExistingAlias.isEmpty else { return next }

                let currentAlias = generatedRelayAlias(for: modelID, relaySlug: relaySlug)
                let currentAliasLower = currentAlias.lowercased()
                if normalizedExistingAlias == currentAliasLower {
                    return next
                }

                let legacyCandidates = legacyRelayAliasCandidates(
                    for: modelID,
                    currentRelaySlug: relaySlug,
                    legacyRelaySlug: legacyRelaySlug
                )
                if legacyCandidates.contains(normalizedExistingAlias) {
                    next.alias = currentAlias
                }
                return next
            }
        }
        ...
        if mappings.isEmpty, let relaySlug = relayChannelSlug {
            // If real models are discovered, expose those directly as aliases.
            // This keeps picker labels close to upstream model IDs and avoids
            // forcing Claude slot defaults onto non-Claude relays.
    ...
    private var relayChannelSlug: String? {
        guard let extracted = extractedRelayChannelSlug else {
            return nil
        }
        return normalizedRelaySlug(extracted)
    }

    /// Relay slug generated by legacy logic before reserved-prefix normalization.
    private var legacyRelayChannelSlug: String? {
        extractedRelayChannelSlug
    }

    /// Best-effort raw relay slug extracted from provider endpoint/name.
    private var extractedRelayChannelSlug: String? {
    ...
    private func normalizedRelaySlug(_ slug: String) -> String {
        guard isReservedRelaySlug(slug) else {
            return slug
        }
        return "relay-\(slug)"
    }

    private func isReservedRelaySlug(_ slug: String) -> Bool {
        switch slug.lowercased() {
        case "anthropic", "claude", "openai", "codex", "gemini",
             "google", "qwen", "glm", "minimax", "kimi", "deepseek",
             "grok", "kiro", "moonshot":
            return true
        default:
            return false
    ...
    private func isGenericRelayPathSegment(_ slug: String) -> Bool {
        switch slug {
        case "api", "v1", "v2", "v3", "relay", "proxy", "anthropic", "openai", "compatible":
            return true
        default:
            return false
        }
    ...
    private func inferredSlotModels(forRelaySlug relaySlug: String) -> RelaySlotModels {
        ...
        if hint.contains("dashscope") || hint.contains("aliyun") || hint.contains("aliyuncs.com/apps/anthropic") {
    ...
    private func generatedRelayAlias(for modelID: String, relaySlug: String) -> String {
        let sanitizedModelID = sanitizeSlug(modelID)
        if isCollisionProneRelayModel(modelID) {
            return "\(relaySlug)-relay-\(sanitizedModelID)"
        }
        return legacyGeneratedRelayAlias(for: modelID, relaySlug: relaySlug)
    }

    private func legacyGeneratedRelayAlias(for modelID: String, relaySlug: String) -> String {
        "\(relaySlug)-\(sanitizeSlug(modelID))"
    }

    private func isCollisionProneRelayModel(_ modelID: String) -> Bool {
    ...
    private func legacyRelayAliasCandidates(
        for modelID: String,
        currentRelaySlug: String,
        legacyRelaySlug: String?
    ) -> Set<String> {
        let sanitizedModelID = sanitizeSlug(modelID)
        var candidates: Set<String> = [
            legacyGeneratedRelayAlias(for: modelID, relaySlug: currentRelaySlug).lowercased()
        ]

        if let legacyRelaySlug, !legacyRelaySlug.isEmpty {
            candidates.insert(legacyGeneratedRelayAlias(for: modelID, relaySlug: legacyRelaySlug).lowercased())
        }

        // Keep compatibility with transient alias scheme variants that existed
        // while iterating on relay routing.
        if isCollisionProneRelayModel(modelID) {
            candidates.insert("\(currentRelaySlug)-relay-\(sanitizedModelID)".lowercased())
            if let legacyRelaySlug, !legacyRelaySlug.isEmpty {
                candidates.insert("\(legacyRelaySlug)-relay-\(sanitizedModelID)".lowercased())
            }
        }
